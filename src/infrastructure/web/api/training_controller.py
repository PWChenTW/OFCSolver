...
class StartTrainingRequest(BaseModel):
    ...
    scenario_type: str = Field(
        default="general", description="Scenario type: general, endgame, fantasy_land"
    )
    session_length: int = Field(
        default=10, description="Number of scenarios in session"
    )
...
class SubmitAnswerRequest(BaseModel):
    ...
    reasoning: Optional[str] = Field(
        default=None, description="Optional reasoning explanation"
    )
...
async def start_training_session(
    request: StartTrainingRequest, command_handler=Depends(get_training_command_handler)
...
async def get_training_session(
    session_id: UUID, query_handler=Depends(get_training_query_handler)
) -> TrainingSessionResponse:
    """
...
async def get_user_performance(
    user_id: UUID, query_handler=Depends(get_training_query_handler)
) -> PerformanceResponse:
    """
