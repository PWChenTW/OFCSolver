...
class Game(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("status IN ('active', 'completed', 'paused', 'cancelled')", name='valid_status'),
...
class Position(Base):
    ...
    analysis_results = relationship("AnalysisResult", back_populates="position", cascade="all, delete-orphan")
    ...
    __table_args__ = (
        ...
        Index('idx_positions_game_round_player', 'game_id', 'round_number', 'current_player', unique=True),
    )
...
class AnalysisResult(Base):
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("calculation_method IN ('heuristic', 'monte_carlo', 'exhaustive', 'hybrid')", 
                       name='valid_calculation_method'),
        CheckConstraint('calculation_time_ms >= 0', name='valid_calculation_time'),
        CheckConstraint('confidence_level BETWEEN 0 AND 1', name='valid_confidence_level'),
...
class TrainingSession(Base):
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("difficulty_level IN ('beginner', 'intermediate', 'advanced', 'expert')", 
                       name='valid_difficulty'),
        CheckConstraint("status IN ('active', 'completed', 'paused', 'abandoned')", 
        ...
        CheckConstraint('completed_scenarios <= total_scenarios', name='completed_not_exceed_total'),
...
class SystemMetric(Base):
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("metric_type IN ('counter', 'gauge', 'histogram', 'summary')", 
