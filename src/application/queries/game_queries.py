                ...
                rounds.append(
                    {
                        ...
                        "fantasy_land_players": [
                            str(p) for p in round_obj.fantasy_land_players
                        ],
                    }
                )

        return cls(**base_dto.__dict__, rounds=rounds, positions=positions)
    ...
    async def handle(self, query: GetActiveGamesQuery) -> GetActiveGamesResult:
        ...
        if query.player_id:
            games = await self.game_repository.find_active_by_player(
                query.player_id, offset=pagination.offset, limit=pagination.limit
            )
