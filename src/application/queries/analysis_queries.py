...
class StrategyRecommendationDTO:
    ...
    @classmethod
    def from_strategy(
        cls, strategy: Strategy, ev: ExpectedValue
...
class GetAnalysisSessionHandler(
    QueryHandler[GetAnalysisSessionQuery, GetAnalysisSessionResult]
):
...
class GetAnalysisHistoryHandler(
    QueryHandler[GetAnalysisHistoryQuery, GetAnalysisHistoryResult]
):
...
class GetAnalysisResultHandler(
    QueryHandler[GetAnalysisResultQuery, GetAnalysisResultResult]
):
    ...
    async def handle(self, query: GetAnalysisResultQuery) -> GetAnalysisResultResult:
        ...
        return GetAnalysisResultResult(
            result=result_dto, visualization_data=visualization_data
        )
    ...
    def _generate_chart_data(self, calculation: Any) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence_chart": {
                ...
                "lower_bounds": [
                    ci[0] for ci in calculation.confidence_intervals.values()
                ],
                "upper_bounds": [
                    ci[1] for ci in calculation.confidence_intervals.values()
                ],
            },
        }
...
class GetStrategyRecommendationsHandler(
    ...
    async def handle(
        self, query: GetStrategyRecommendationsQuery
    ) -> GetStrategyRecommendationsResult:
        """Handle the query."""
        ...
        position_summary = await self._get_position_summary(
            query.session_id, query.position_id
        )

        return GetStrategyRecommendationsResult(
            recommendations=recommendations, position_summary=position_summary
        )

    async def _get_position_summary(
        self, session_id: UUID, position_id: UUID
    ) -> Dict[str, Any]:
...
class CompareAnalysisResultsHandler(
    ...
    async def handle(
        self, query: CompareAnalysisResultsQuery
    ) -> CompareAnalysisResultsResult:
        """Handle the query."""
        # Get all sessions
        sessions = []
        ...
        if not sessions:
            return CompareAnalysisResultsResult(
                comparison=AnalysisComparisonDTO(
                    session_ids=[], metrics={}, summary={}, charts=[]
                )
            )
    ...
    def _generate_comparison_summary(
        self, sessions: List[Any], metrics: Dict[str, List[float]]
    ) -> Dict[str, Any]:
        """Generate comparison summary."""
        return {
            ...
            "best_performers": {
                metric: (
                    max(range(len(values)), key=values.__getitem__) if values else -1
                )
                for metric, values in metrics.items()
            },
        }
