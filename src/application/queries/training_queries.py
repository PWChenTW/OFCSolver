...
class ProgressDTO:
    ...
    @classmethod
    def from_performance_data(
        cls, user_id: UUID, performance_data: Dict[str, Any]
    ) -> "ProgressDTO":
...
class GetTrainingSessionHandler(
    QueryHandler[GetTrainingSessionQuery, GetTrainingSessionResult]
):
    ...
    async def handle(self, query: GetTrainingSessionQuery) -> GetTrainingSessionResult:
        ...
        if not session:
            return GetTrainingSessionResult(
                session=None, exercises=None, performance_summary=None
            )
...
class GetUserProgressHandler(QueryHandler[GetUserProgressQuery, GetUserProgressResult]):
    ...
    async def handle(self, query: GetUserProgressQuery) -> GetUserProgressResult:
        ...
        progress_dto = ProgressDTO.from_performance_data(
            query.user_id, performance_data
        )
    ...
    async def _calculate_performance_data(
        ...
        return {
            ...
            "average_accuracy": (
                sum(s.get_average_accuracy() for s in completed_sessions)
    ...
    async def _calculate_metric(
        self, sessions: List[TrainingSession], metric: ProgressMetric
    ) -> Dict[str, float]:
        """Calculate specific metric values."""
        # Placeholder implementation
        return {"current": 0.0, "average": 0.0, "best": 0.0, "trend": 0.0}
    ...
    def _group_progress_by_difficulty(
        self, sessions: List[TrainingSession]
    ...
    def _group_progress_by_scenario(
        self, sessions: List[TrainingSession]
    ) -> Dict[str, Dict[str, float]]:
...
class GetTrainingHistoryHandler(
    QueryHandler[GetTrainingHistoryQuery, GetTrainingHistoryResult]
):
    ...
    async def handle(self, query: GetTrainingHistoryQuery) -> GetTrainingHistoryResult:
        ...
        return GetTrainingHistoryResult(
            sessions=paginated_result, summary_stats=summary_stats
        )
...
class GetScenarioStatsHandler(
    QueryHandler[GetScenarioStatsQuery, GetScenarioStatsResult]
):
    ...
    async def _calculate_scenario_stats(
        self, scenario: Scenario, sessions: List[TrainingSession]
    ) -> ScenarioStatsDTO:
        ...
        return ScenarioStatsDTO(
            ...
            completion_rate=(
                len([s for s in sessions if s.is_completed]) / len(sessions)
                if sessions
                else 0
            ),
            average_score=(
                sum(s.score for s in sessions) / len(sessions) if sessions else 0
            ),
            average_time=(
                sum(s.get_duration() for s in sessions) / len(sessions)
                if sessions
                else 0
            ),
...
class GetLeaderboardHandler(QueryHandler[GetLeaderboardQuery, GetLeaderboardResult]):
    ...
    async def handle(self, query: GetLeaderboardQuery) -> GetLeaderboardResult:
        ...
        entries, total, time_period = await self._get_leaderboard_data(
            query.leaderboard_type, query.scenario_id, query.time_range, pagination
        )

        # Convert to DTOs
        entry_dtos = [
            self._create_leaderboard_entry(i + 1, entry)
        ...
        return GetLeaderboardResult(
            entries=paginated_result, user_rank=user_rank, time_period=time_period
        )
    ...
    def _create_leaderboard_entry(
        self, rank: int, data: Dict[str, Any]
    ) -> LeaderboardEntryDTO:
...
class GetPersonalBestsHandler(
    QueryHandler[GetPersonalBestsQuery, GetPersonalBestsResult]
):
    ...
    async def handle(self, query: GetPersonalBestsQuery) -> GetPersonalBestsResult:
        ...
        best_sessions = await self.training_repository.find_best_by_user(
            query.user_id, scenario_ids=query.scenario_ids, limit=query.limit
        )
...
class GetRecommendedScenariosHandler(
    ...
    async def handle(
        self, query: GetRecommendedScenariosQuery
    ) -> GetRecommendedScenariosResult:
        """Handle the query."""
        # Get user's training history
        user_sessions = await self.training_repository.find_by_user(
            query.user_id, limit=50  # Last 50 sessions for analysis
        )
        ...
        recommended_scenarios = await self._find_matching_scenarios(
            user_profile, query.difficulty_preference, query.max_recommendations
        )
            ...
            recommendations.append(
                ScenarioRecommendationDTO(
                    ...
                    recommended_difficulty=self._suggest_difficulty(
                        user_profile, scenario, query.difficulty_preference
                    ),
                    reason=reason,
    ...
    async def _find_matching_scenarios(
        self,
    ...
    def _suggest_difficulty(
        ...
        user_profile: Dict[str, Any],
        scenario: Scenario,
        preference: Optional[DifficultyLevel],
    ) -> DifficultyLevel:
    ...
    def _estimate_completion_time(
        self, user_profile: Dict[str, Any], scenario: Scenario
    ) -> float:
