...
from ...value_objects import Card, GameRules, Hand, Move
...
class Position(DomainEntity):
    ...
    def is_terminal_position(self) -> bool:
        ...
        return all(hand.is_complete() for hand in self._players_hands.values())
    ...
    def apply_move(self, move: Move) -> "Position":
        ...
        # Apply move to current player's hand (placeholder - Hand.place_card needs
        # implementation)
        # new_current_hand = current_hand.place_card(move.card, move.position)
        # new_hands[self._current_player_id] = new_current_hand
        # For now, just use the existing hand
        new_hands[self._current_player_id] = current_hand
    ...
    def to_analysis_format(self) -> Dict:
        ...
        return {
            ...
            "players_hands": {
                pid: hand.to_dict() for pid, hand in self._players_hands.items()
            },
    ...
    def _generate_position_id(
        ...
        position_data += ":".join(
            sorted([f"{pid}:{hand.to_string()}" for pid, hand in players_hands.items()])
        )
        position_data += ":" + ":".join(sorted(str(card) for card in remaining_cards))

        # Generate hash-based ID
        hash_digest = hashlib.md5(
            position_data.encode(), usedforsecurity=False
        ).hexdigest()
    ...
    def _calculate_complexity_score(self) -> float:
        ...
        incomplete_hands = sum(
            1 for hand in self._players_hands.values() if not hand.is_complete()
        )
        hands_factor = incomplete_hands / len(self._players_hands)

        # Combine factors
        complexity = (
            base_score * cards_factor * phase_factor * moves_factor * hands_factor
        )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"Position(id={self.id}, game={self._game_id}, "
