...
from ...value_objects import Card, Hand, Score, GameRules, Deck, CardPosition
from ...events import GameCompletedEvent, CardPlacedEvent, RoundStartedEvent
from ...exceptions import InvalidCardPlacementError, GameStateError
...
class CardPlacedEvent(DomainEvent):
    """Event fired when a card is placed in the game."""
    game_id: GameId
    player_id: PlayerId
    card: Card
...
class Game(AggregateRoot):
    ...
    def place_card(self, player_id: PlayerId, card: Card, position: CardPosition) -> None:
        ...
        # Emit domain event
        self.add_domain_event(CardPlacedEvent(
            game_id=str(self.id),
            player_id=player_id,
            card=card,
            position=position,
    ...
    def get_analysis_position(self) -> 'Position':
        ...
        from .position import Position  # Avoid circular import
    ...
    def _start_round(self, round_number: int) -> None:
        ...
        self.add_domain_event(RoundStartedEvent(
            game_id=str(self.id),
            round_number=round_number,
            active_player_id=current_player.id
        ))
    
    def _can_place_card(self, player: Player, card: Card, position: CardPosition) -> bool:
    ...
    def _complete_game(self) -> None:
        ...
        self.add_domain_event(GameCompletedEvent(
            game_id=str(self.id),
