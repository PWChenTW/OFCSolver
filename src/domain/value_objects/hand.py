...
class Hand(ValueObject):
    ...
    def __post_init__(self):
        ...
        if len(self.top_row) > 3:
            raise HandValidationError(
                f"Top row cannot have more than 3 cards, got {len(self.top_row)}"
            )
        if len(self.middle_row) > 5:
            raise HandValidationError(
                f"Middle row cannot have more than 5 cards, got {len(self.middle_row)}"
            )
        if len(self.bottom_row) > 5:
            raise HandValidationError(
                f"Bottom row cannot have more than 5 cards, got {len(self.bottom_row)}"
            )
        ...
        if len(all_cards) > 13:
            raise HandValidationError(
                f"Hand cannot contain more than 13 cards, got {len(all_cards)}"
            )
    ...
    def from_cards(cls, cards: List[Card]) -> "Hand":
        ...
        if len(cards) > 13:
            raise HandValidationError(
                f"Cannot create hand with more than 13 cards, got {len(cards)}"
            )
    ...
    def place_card(self, card: Card, position: CardPosition) -> "Hand":
        ...
        if not self.can_place_card(position):
            raise InvalidCardPlacementError(
                f"Cannot place card in {position} - position full"
            )
