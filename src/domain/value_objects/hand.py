...
class Hand(ValueObject):
    ...
    def is_complete(self) -> bool:
        ...
        return (len(self.top_row) == 3 and 
                len(self.middle_row) == 5 and 
                len(self.bottom_row) == 5)
    ...
    def to_string(self) -> str:
        ...
        return f"T:{' '.join(str(c) for c in self.top_row)} M:{' '.join(str(c) for c in self.middle_row)} B:{' '.join(str(c) for c in self.bottom_row)}"
