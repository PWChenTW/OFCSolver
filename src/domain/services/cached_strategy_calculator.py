...
class CachedStrategyCalculator(StrategyCalculator):
    ...
    def calculate_optimal_strategy(
        ...
        position_data = self._create_position_data(current_hand, remaining_deck, max_depth)
        position_hash = self.cache_manager.key_builder.hash_position(position_data)
        
        # Check cache first
        cached_analysis = self.cache_manager.get_analysis(position_hash, "minimax_alphabeta")
        if cached_analysis:
        ...
        # Calculate strategy
        strategy = super().calculate_optimal_strategy(current_hand, remaining_deck, max_depth)
    ...
    def _create_analysis_result(
        ...
        if strategy.recommended_actions:
            ...
            result["recommended_action"] = {
                ...
                "card_to_discard": str(action.card_to_discard) if action.card_to_discard else None,
        ...
        if strategy.recommended_actions:
            ...
            for action in strategy.recommended_actions:
                data["actions"].append({
                    ...
                    "discard": str(action.card_to_discard) if action.card_to_discard else None,
    ...
    def _reconstruct_strategy(self, cached_data: Dict[str, Any]) -> Strategy:
        ...
        if "recommended_action" in cached_data:
            ...
            discard = Card.from_string(action_data["card_to_discard"]) if action_data["card_to_discard"] else None
    ...
    def warm_cache_for_position(
        ...
        for i in range(min(variations, len(deck_copy))):
            ...
            try:
                strategy = self.calculate_optimal_strategy(hand, varied_deck, max_depth=1)
                positions_cached += 1
            except Exception as e:
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        perf_stats.update({
            ...
            "redis_position_hit_rate": (
                ...
                (self._cache_stats["position_hits"] + self._cache_stats["position_misses"])
                if (self._cache_stats["position_hits"] + self._cache_stats["position_misses"]) > 0
                else 0.0
            ),
            "redis_strategy_hit_rate": (
                self._cache_stats["strategy_hits"] /
                (self._cache_stats["strategy_hits"] + self._cache_stats["strategy_misses"])
                if (self._cache_stats["strategy_hits"] + self._cache_stats["strategy_misses"]) > 0
                else 0.0
            )
        })
