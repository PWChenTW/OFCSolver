...
class GameValidator(DomainService):
    ...
    def validate_row_strength_progression(self, player: Player) -> ValidationResult:
        ...
        if player.status == PlayerStatus.FOULED:
            return ValidationResult(
                is_valid=False,
                error_message="Player has fouled hand"
            )
    ...
    def check_game_completion(self, game: Game) -> ValidationResult:
        ...
        if game.is_completed:
            return ValidationResult(
                is_valid=True,
                warning_message="Game is already completed"
            )
    ...
    def validate_turn_order(self, game: Game, player_id: PlayerId) -> ValidationResult:
        ...
        if game.is_completed:
            return ValidationResult(
                is_valid=False,
                error_message="Game is completed, no more turns"
            )
    ...
    def validate_multi_player_game_state(self, game: Game) -> ValidationResult:
        ...
        if len(set(player_ids)) != len(player_ids):
            return ValidationResult(
                is_valid=False,
                error_message="Duplicate player IDs found"
            )
        ...
        if len(all_placed_cards) != len(set(all_placed_cards)):
            return ValidationResult(
                is_valid=False,
                error_message="Duplicate cards found across players"
            )
    ...
    def can_place_card_safely(
        ...
        if (len(current_top) == 3 and len(current_middle) >= 3 and len(current_bottom) >= 3):
            # Check if this placement would violate progression
            top_hand = self._hand_evaluator.evaluate_hand(current_top)
            middle_partial = self._hand_evaluator.evaluate_hand(current_middle[:3])  # Compare with same number of cards
    ...
    def get_available_positions(self, game: Game, player_id: PlayerId) -> List[CardPosition]:
        """
