...
class GameTreeBuilder(DomainService):
    ...
    def _expand_node(
        ...
        for i, (card1, card2) in enumerate(possible_placements):
            ...
            child_hand = self._try_place_cards(
                node.player_hand,
                [card1, card2]
            )
    ...
    def _try_place_cards(
        ...
        if len(hand.middle_row) < 5:
            available_positions.extend([CardPosition.MIDDLE] * (5 - len(hand.middle_row)))
        if len(hand.bottom_row) < 5:
            available_positions.extend([CardPosition.BOTTOM] * (5 - len(hand.bottom_row)))
