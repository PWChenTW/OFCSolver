...
class StrategyCalculator(DomainService):
    ...
    def _minimax(
        ...
        if depth == 0 or node.is_terminal or node.is_leaf:
            ...
            self.evaluations[node.node_id] = NodeEvaluation(
                node_id=node.node_id, value=eval_value, visits=1
            )
        ...
        if maximizing_player:
            ...
            self.evaluations[node.node_id] = NodeEvaluation(
                node_id=node.node_id, value=max_eval, best_action=best_action, visits=1
            )
            # Cache the result
            if position_key:
            ...
            self.evaluations[node.node_id] = NodeEvaluation(
                node_id=node.node_id, value=min_eval, best_action=best_action, visits=1
            )
            # Cache the result
            if position_key:
    ...
    def _get_child_nodes(
        self, node: GameTreeNode
    ) -> List[Tuple[GameTreeNode, NodeAction]]:
    ...
    def _is_risky_position(self, hand: Hand) -> bool:
        ...
        if hand.top_row and hand.middle_row and hand.bottom_row:
            ...
            top_eval = (
                self.evaluator.evaluate_hand(hand.top_row)
                if len(hand.top_row) >= 3
                else None
            )
            middle_eval = (
                self.evaluator.evaluate_hand(hand.middle_row)
                if len(hand.middle_row) >= 5
                else None
            )
            bottom_eval = (
                self.evaluator.evaluate_hand(hand.bottom_row)
                if len(hand.bottom_row) >= 5
                else None
            )
    ...
    def calculate_ev_range(
        ...
        if len(ev_samples) >= 30:
            ...
            lower_bound = (
                ...
                if lower_idx < len(sorted_samples)
                else sorted_samples[0]
            )
            upper_bound = (
                sorted_samples[upper_idx - 1] if upper_idx > 0 else sorted_samples[-1]
            )
    ...
    def _evaluate_row_balance(self, hand: Hand) -> float:
        ...
        row_sizes = [len(hand.top_row), len(hand.middle_row), len(hand.bottom_row)]
        ...
        deviation = sum(
            abs(ideal - actual) for ideal, actual in zip(ideal_ratios, actual_ratios)
        )

        # Return score (1.0 = perfect balance, 0.0 = worst)
        return max(0.0, 1.0 - deviation * 2.0)

    def _calculate_fantasy_land_probability(
        self, hand: Hand, cards_placed: int
    ) -> float:
    ...
    def _calculate_risk_factor(self, hand: Hand) -> float:
        ...
        if len(hand.middle_row) >= 5:
            middle_eval = self.evaluator.evaluate_hand(hand.middle_row)
            strengths.append(
                ("middle", middle_eval.hand_type, middle_eval.strength_value)
            )

        if len(hand.bottom_row) >= 5:
            bottom_eval = self.evaluator.evaluate_hand(hand.bottom_row)
            strengths.append(
                ("bottom", bottom_eval.hand_type, bottom_eval.strength_value)
            )
