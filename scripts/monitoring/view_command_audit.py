...
class CommandAuditViewer:
    ...
    def _classify_command(self, command):
        ...
        if any(
            word in command_lower for word in ["git", "commit", "push", "pull", "merge"]
        ):
            return "git_operation"
        elif any(
            word in command_lower for word in ["test", "pytest", "unittest", "jest"]
        ):
            return "testing"
        elif any(
            word in command_lower for word in ["npm", "pip", "yarn", "cargo", "go mod"]
        ):
            return "package_management"
        elif any(
            word in command_lower for word in ["python", "node", "go run", "java"]
        ):
            return "development"
            ...
            word in command_lower for word in ["ls", "cat", "grep", "find", "mkdir"]
        ):
    ...
    def analyze_patterns(self, hours=24):
        ...
        # 最近的命令
        print(f"\n🕒 最近 10 個命令:")
        for entry in sorted(recent_entries, key=lambda x: x["timestamp"], reverse=True)[
            :10
        ]:
    ...
    def show_summary(self):
        ...
        print(f"總命令數: {total_commands}")
        print(
            f"時間範圍: {first_command.strftime('%Y-%m-%d %H:%M')} 至 {last_command.strftime('%Y-%m-%d %H:%M')}"
        )
...
def main():
    ...
    parser.add_argument(
        "--top-commands", type=int, metavar="N", help="顯示最常用的N個命令"
    )
