...
def upgrade() -> None:
    ...
    op.create_table('games',
        ...
        sa.CheckConstraint("status IN ('active', 'completed', 'paused', 'cancelled')", name='valid_status'),
    ...
    op.create_table('positions',
        ...
        sa.ForeignKeyConstraint(['game_id'], ['games.id'], name='fk_positions_game_id_games', ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id', name='pk_positions')
    )
    op.create_index('idx_positions_created_at', 'positions', ['created_at'], unique=False)
    op.create_index('idx_positions_game_id', 'positions', ['game_id'], unique=False)
    op.create_index('idx_positions_game_round_player', 'positions', ['game_id', 'round_number', 'current_player'], unique=True)
    ...
    op.create_table('analysis_results',
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=True),
        sa.CheckConstraint("calculation_method IN ('heuristic', 'monte_carlo', 'exhaustive', 'hybrid')", name='valid_calculation_method'),
        sa.CheckConstraint('calculation_time_ms >= 0', name='valid_calculation_time'),
        sa.CheckConstraint('confidence_level BETWEEN 0 AND 1', name='valid_confidence_level'),
        sa.ForeignKeyConstraint(['position_id'], ['positions.id'], name='fk_analysis_results_position_id_positions', ondelete='CASCADE'),
    ...
    op.create_index('idx_analysis_method', 'analysis_results', ['calculation_method'], unique=False)
    op.create_index('idx_analysis_position_id', 'analysis_results', ['position_id'], unique=False)
    ...
    op.create_table('training_sessions',
        ...
        sa.Column('completed_at', sa.DateTime(timezone=True), nullable=True),
        sa.CheckConstraint('completed_scenarios <= total_scenarios', name='completed_not_exceed_total'),
        sa.CheckConstraint('completed_scenarios >= 0', name='valid_completed_scenarios'),
        sa.CheckConstraint("difficulty_level IN ('beginner', 'intermediate', 'advanced', 'expert')", name='valid_difficulty'),
        sa.CheckConstraint("status IN ('active', 'completed', 'paused', 'abandoned')", name='valid_session_status'),
        sa.CheckConstraint('total_scenarios > 0', name='valid_total_scenarios'),
        sa.PrimaryKeyConstraint('id', name='pk_training_sessions')
    )
    op.create_index('idx_training_started_at', 'training_sessions', ['started_at'], unique=False)
    op.create_index('idx_training_status', 'training_sessions', ['status'], unique=False)
    op.create_index('idx_training_user_id', 'training_sessions', ['user_id'], unique=False)
    ...
    op.create_table('performance_metrics',
        ...
        sa.ForeignKeyConstraint(['session_id'], ['training_sessions.id'], name='fk_performance_metrics_session_id_training_sessions', ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id', name='pk_performance_metrics')
    )
    op.create_index('idx_performance_created_at', 'performance_metrics', ['created_at'], unique=False)
    op.create_index('idx_performance_is_correct', 'performance_metrics', ['is_correct'], unique=False)
    op.create_index('idx_performance_session_id', 'performance_metrics', ['session_id'], unique=False)
    ...
    op.create_table('system_metrics',
        ...
        sa.CheckConstraint("metric_type IN ('counter', 'gauge', 'histogram', 'summary')", name='valid_metric_type'),
    ...
    op.create_index('idx_system_metrics_name_time', 'system_metrics', ['metric_name', 'recorded_at'], unique=False)
