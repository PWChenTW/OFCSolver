...
def create_mock_games(count: int) -> List[Game]:
    ...
    for i in range(count):
        ...
        game.completed_at = (
            None if game.status == GameStatus.IN_PROGRESS else game.updated_at
        )
        game.current_round = i % 10 + 1
        game.current_player_id = (
            uuid.uuid4() if game.status == GameStatus.IN_PROGRESS else None
        )
...
def create_mock_sessions(count: int) -> List[AnalysisSession]:
    ...
    for i in range(count):
        ...
        session.status = (
            CalculationStatus.COMPLETED if i % 3 == 0 else CalculationStatus.IN_PROGRESS
        )
        session.created_at = datetime.now() - timedelta(days=i)
        session.started_at = session.created_at + timedelta(minutes=1)
        session.completed_at = (
            session.started_at + timedelta(minutes=i * 2)
            if session.status == CalculationStatus.COMPLETED
            else None
        )
...
def create_mock_training_sessions(count: int) -> List[TrainingSession]:
    ...
    for i in range(count):
        ...
        session.completed_at = (
            session.started_at + timedelta(minutes=30) if i % 2 == 0 else None
        )
        ...
        for filters in test_cases:
            for _ in range(10):  # Run each query 10 times
                query = GetGameHistoryQuery(
                    pagination=PaginationParams(page=1, page_size=50), **filters
                )
...
class TestQueryOptimizationMiddleware:
    ...
    async def test_middleware_auto_optimization(
        self, query_optimizer, mock_repositories
    ):
