...
class TestFantasyLandManager:
    ...
    def test_qualifies_multiple_rows(self):
        ...
        assert self.manager.qualifies_for_fantasy_land(
            top_hand=top_hand, middle_hand=middle_hand
        )
    ...
    def test_cannot_stay_weak_hands(self):
        ...
        assert not self.manager.can_stay_in_fantasy_land(
            top_hand=top_hand, middle_hand=middle_hand, bottom_hand=bottom_hand
        )
    ...
    def test_validate_fantasy_land_setting_duplicates(self):
        ...
        cards = Card.parse_cards(
            "As Ks Qs Js Ts 9s 8s 7s 6s 5s 4s 3s As"
        )  # As appears twice
        assert not self.manager.validate_fantasy_land_setting(cards, "standard")
    ...
    def test_get_qualification_requirements(self):
        ...
        assert (
            requirements["initial_qualification"]["top"]
            == "QQ or better (pair of queens or higher)"
        )
        assert (
            requirements["initial_qualification"]["middle"]
    ...
    def test_calculate_fantasy_land_probability(self):
        ...
        middle_prob = self.manager.calculate_fantasy_land_probability(
            cards_seen, "middle"
        )
        ...
        bottom_prob = self.manager.calculate_fantasy_land_probability(
            cards_seen, "bottom"
        )
        ...
        # Invalid position
        invalid_prob = self.manager.calculate_fantasy_land_probability(
            cards_seen, "invalid"
        )
